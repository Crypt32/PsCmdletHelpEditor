...
namespace CodeKicker.BBCode {
	...
	public sealed class BBCodeParser {
		...
		public string ToHtml(string bbCode, Boolean xml = false) {
			if (String.IsNullOrEmpty(bbCode)) { return null; }
			if (bbCode == null) throw new ArgumentNullException("bbCode");
			return xml
				? ParseSyntaxTree(bbCode).ToHtml()
				: ParseSyntaxTree(bbCode).ToHtml().Replace("    ", "&nbsp;&nbsp;&nbsp;&nbsp;");
		}

		private SequenceNode ParseSyntaxTree(string bbCode) {
			if (bbCode == null) throw new ArgumentNullException("bbCode");
